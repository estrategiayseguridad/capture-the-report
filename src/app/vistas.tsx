  ...
  return (
    <div>
          ...
          >
            ...
            <span
              className={`ml-2 rounded px-1.5 py-0.5 text-xs ${
                g.herramienta === activa ? "bg-blue-500" : "bg-slate-100"
              }`}
            >
              {g.tickets.length}
            </span>
      ...
      <p className="mt-3 text-xs text-slate-500">
        Pestana generada desde <code className="text-slate-700">Category</code>:{" "}
        <strong className="text-slate-800">{grupo.tickets.length} tickets</strong>. El proceso
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100">
            {grupo.tickets.map((t) => {
              ...
              return (
                <tr key={t.ticket_id} className={perdido ? "bg-red-50/60" : "bg-white"}>
                  <td className="px-3 py-1.5 font-mono text-slate-700">{t.ticket_id}</td>
                  <td className="max-w-md truncate px-3 py-1.5 text-slate-800" title={t.summary}>
                    {t.summary}
                  </td>
...
function EtiquetaEstado({ estado }: { estado: string }) {
  const colores: Record<string, string> = {
    ...
    "With User": "bg-amber-100 text-amber-800",
  ...
  return (
    <section className="rounded-xl border-2 border-red-300 bg-red-50 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-sm font-bold tracking-wide text-red-900 uppercase">
            ...
            <tbody className="divide-y divide-red-100">
              {tickets.map((t) => (
                <tr key={t.ticket_id}>
                  ...
                  <td className="px-3 py-1.5 text-slate-500">{t.category.split(">")[0]}</td>
                </tr>
              ))}
            </tbody>
          </table>
